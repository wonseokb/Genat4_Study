...
G4bool LXeDetectorConstruction::fSphereOn = true;//For Sphere Geometry

//============================================================================================================================================

LXeDetectorConstruction::LXeDetectorConstruction(): fLXe_mt(nullptr), fMPTPStyrene(nullptr)
{
  fExperimentalHall_box = nullptr;//nullptr문법 진짜 요상하네.
  ...
  SetDefaults();
  DefineMaterials();
  fDetectorMessenger = new LXeDetectorMessenger(this);//check needed
}
...
G4VPhysicalVolume* LXeDetectorConstruction::Construct(){
  ...
  G4double expHall_x = fScint_x+fD_mtl+1.*m;//check
  ...
  //Place the WLS slab
  if(fWLSslab){ 
    G4VPhysicalVolume* slab = new LXeWLSSlab(0,G4ThreeVector(0.,0., -fScint_z/2.-fSlab_z-1.*cm), fExperimentalHall_log,false,0, this);
