...
G4VPhysicalVolume* WLSDetectorConstruction::ConstructDetector()
{
  ...
  // Scintillator
  G4VSolid* solidScintillator = new G4Box("Scintillator", GetBarBase()/2-GetCoatingThickness() -GetCoatingRadius(), GetBarBase()/2-GetCoatingThickness() -GetCoatingRadius(), GetBarLength()/2);
  ...
  if (GetCoatingRadius() > 0.*mm) {
     G4VSolid* solidScintside = new G4Box("SideOfBar", GetBarBase()/2-GetCoatingThickness() -GetCoatingRadius(), GetCoatingRadius()/2, GetBarLength()/2);
     ...
     new G4PVPlacement(0, G4ThreeVector(0,-y,0), logicScintSide, "SideOfBar", logicExtrusion, false, 0);
     new G4PVPlacement(0, G4ThreeVector(0, y,0), logicScintSide, "SideOfBar", logicExtrusion, false, 1);

     G4RotationMatrix* g4rot = new G4RotationMatrix();
     *g4rot = StringToRotationMatrix("Z90");
     *g4rot = g4rot->inverse();
     if (*g4rot == G4RotationMatrix()) g4rot = NULL;
     new G4PVPlacement(g4rot, G4ThreeVector(x,0,0), logicScintSide, "SideOfBar", logicExtrusion, false, 2);
     new G4PVPlacement(g4rot, G4ThreeVector(-x,0,0), logicScintSide, "SideOfBar", logicExtrusion, false, 3);

     x = GetBarBase()/2-GetCoatingThickness()-GetCoatingRadius();
     y = GetBarBase()/2-GetCoatingThickness()-GetCoatingRadius();
     new G4PVPlacement(0, G4ThreeVector(x,y,0), logicScintCrnr, "CrnrOfBar", logicExtrusion, false, 0);
     new G4PVPlacement(g4rot, G4ThreeVector(-x,y,0), logicScintCrnr, "CrnrOfBar", logicExtrusion, false, 1);

     g4rot = new G4RotationMatrix();
     *g4rot = StringToRotationMatrix("Z180");
     *g4rot = g4rot->inverse();
     if (*g4rot == G4RotationMatrix()) g4rot = NULL;
     new G4PVPlacement(g4rot, G4ThreeVector(-x,-y,0), logicScintCrnr, "CrnrOfBar", logicExtrusion, false, 2);

     g4rot = new G4RotationMatrix();
     *g4rot = StringToRotationMatrix("Z270");
     *g4rot = g4rot->inverse();
     if (*g4rot == G4RotationMatrix()) g4rot = NULL;
     new G4PVPlacement(g4rot, G4ThreeVector(x,-y,0), logicScintCrnr, "CrnrOfBar", logicExtrusion, false, 3);
  }
  ...
  if (GetFiberRadius()<GetHoleRadius()) {
        ...
        fPhysiHole = new G4PVPlacement(0, G4ThreeVector(), fLogicHole, "Hole", logicScintillator, false, 0);
  }
...
void WLSDetectorConstruction::ConstructFiber()
{
  ...
  // Surface Name, SetModel, SetFinish, SetType, SetPolish
  ...
  switch ( fNumOfCladLayers ) {
    ...
    default:
     ...
     if (fXYRatio == 1.)
       solidWLSfiber = new G4Tubs("WLSFiber",0.,fWLSfiberRX,fWLSfiberZ,0.0*rad,twopi*rad);
     else
     ...
     G4VPhysicalVolume* physiWLSfiber = new G4PVPlacement(0, G4ThreeVector(0.0,0.0,fWLSfiberOrigin), logicWLSfiber, "WLSFiber", logicPlacement, false, 0);
  ...
  if (fMirrorToggle) {  
     ...
     new G4PVPlacement(0, G4ThreeVector(0.0,0.0,fMirrorOrigin), logicMirror, "Mirror", fLogicWorld, false, 0);
  ...
  G4LogicalVolume*   logicCouple = new G4LogicalVolume(solidCouple, FindMaterial("G4_AIR"), "Couple");
  new G4PVPlacement(0, G4ThreeVector(0.0,0.0,fCoupleOrigin), logicCouple, "Couple", fLogicWorld, false, 0);
  ...
  new G4PVPlacement(new G4RotationMatrix(CLHEP::HepRotationY(-fMPPCTheta)), G4ThreeVector(fMPPCOriginX,0.0,fMPPCOriginZ), logicClrfiber, "ClearFiber", logicCouple, false, 0);
  ...
  if ( fMPPCShape == "Square" )
    ...
    solidPhotonDet = new G4Tubs("PhotonDet",0.,fMPPCHalfL,fMPPCZ,0.0*rad,twopi*rad);
  G4LogicalVolume*   logicPhotonDet = new G4LogicalVolume(solidPhotonDet, FindMaterial("G4_Al"), "PhotonDet_LV");
...
//===========================================================================================================All below here is set variables
